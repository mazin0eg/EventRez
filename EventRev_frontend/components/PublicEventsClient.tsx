...
/**
 * ============================================
 * CLIENT COMPONENT - Interactivité côté navigateur
 * ============================================
 * 
 * Ce composant utilise 'use client' car il a besoin de:
 * - useAuth() : Vérifier si l'utilisateur est connecté
 * - useState() : Gérer l'état local (loading, messages)
 * - onClick handlers : Gérer les clics de réservation
 * 
 * Les données initiales viennent du SERVEUR (initialEvents)
 * mais l'interactivité se passe dans le NAVIGATEUR
 */
...
export default function PublicEventsClient({ initialEvents }: PublicEventsClientProps) {
  const { user, isAuthenticated, isLoading: authLoading } = useAuth();
  const router = useRouter();
  
  // États locaux pour l'interactivité
  ...
  // Charger la disponibilité côté client (nécessite auth)
  useEffect(() => {
    const fetchAvailability = async () => {
      if (!isAuthenticated) return;

      const availabilityPromises = initialEvents.map(async (event) => {
        try {
                ...
                <div className="p-5">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <Link
                      ...
                      className="text-base font-normal tracking-wide text-stone-800 dark:text-stone-100 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors"
                    >
                      {event.title}
                    </Link>
                    {event.category && (
                  ...
                  <div className="space-y-2 text-xs font-light">
                    ...
                    {/* Disponibilité - seulement si authentifié */}
                      ...
                      <div className="flex items-center text-stone-500 dark:text-stone-400">
                        ...
                        {eventAvailability.available !== null ? (
                          <span className={isSoldOut ? 'text-red-500' : ''}>
                            {eventAvailability.available} / {eventAvailability.capacity} places
                          </span>
                        ) : (
                          'Places illimitées'
                        )}
                  ...
                  <div className="mt-5 pt-4 border-t border-stone-100 dark:border-stone-800">
                    {!isAuthenticated ? (
                      ...
                      <Link
                        href="/login"
                        className="block w-full py-2.5 px-4 text-center bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 font-light text-sm tracking-wide hover:bg-emerald-700 hover:text-white transition-colors"
                      >
                        Connectez-vous pour réserver
                      </Link>
                    ) : user?.role === 'user' ? (
                      // Utilisateur connecté - Bouton de réservation
                      <button
                        onClick={() => handleReserve(event.id)}
                        disabled={isPast || isSoldOut || reservingEventId === event.id || authLoading}
                      ...
                      // Admin - Lien vers détails
                      <Link
                        href={`/events/${event.id}`}
                        className="block w-full py-2.5 px-4 text-center bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 font-light text-sm tracking-wide hover:bg-stone-200 dark:hover:bg-stone-700 transition-colors"
                      >
                        Voir les détails
                      </Link>
                    )}
